...
{
	this->~lbiguint();//Allowed, and accurately represents what we are doing here.
	...
	node * rtemp = l.head->link(), *wtemp = head;//readtemp and writetemp
	while (rtemp != nullptr) {
// 		std::cout << temp
		wtemp->set_link(new node(rtemp->data(), nullptr));
		wtemp = wtemp->link();
		rtemp = rtemp->link();
	}
	tail = wtemp;
